//! Actix security middleware and request adapters.
...
impl RequireAnonClient {
    /// Construct middleware with a header name and expected key.
...
/// Per-service anonymous client key validator.
        ...
        Box::pin(async move {
            if !request_has_valid_anon_client_key(
                ...
                let response = actix_web::HttpResponse::Unauthorized().json(serde_json::json!({
                    "code": "invalid_anon_client",
                    "message": "Missing or invalid anonymous client key"
                }));
...
/// Build anonymous client middleware from the default environment setting.
...
/// Resolve the anonymous client key, falling back to the built-in value.
...
/// Build Actix security response headers from shared settings.
...
/// Build Actix CORS middleware from shared settings.
...
/// Resolve the caller IP using configured trusted proxies.
pub fn request_client_ip(req: &HttpRequest, security: &SecurityConfig) -> Option<IpAddr> {
    ...
    let mut headers = crate::http::HeaderFields::default();
    for (name, value) in req.headers() {
        if headers.append(name.as_str(), value.as_bytes()).is_err() {
            return Some(peer.ip());
        }
    }
    super::resolve_client_ip(Some(peer), &headers, &trusted_proxies)
}
...
mod tests {
    ...
    #[actix_web::test]
    async fn anonymous_client_middleware_keeps_the_error_response_shape() {
        let middleware = super::RequireAnonClient::new(
            header::HeaderName::from_static(DEFAULT_ANON_CLIENT_HEADER_NAME),
            "expected",
        );
        ...
        let body: serde_json::Value = test::read_body_json(denied).await;
        assert_eq!(
            body,
            serde_json::json!({
                "code": "invalid_anon_client",
        ...
        let allowed = test::call_service(
            ...
            test::TestRequest::get()
                .uri("/items")
                .insert_header((DEFAULT_ANON_CLIENT_HEADER_NAME, "expected"))
                .to_request(),
        )
        .await;
        assert_eq!(allowed.status(), actix_web::http::StatusCode::OK);
    }
}
