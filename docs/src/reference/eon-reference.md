...
List settings tune generated list endpoint defaults, count endpoint generation, and explicitly allowed multi-value filters.
...
List endpoints accept `limit=0` as a supported count-only list request. The response keeps the normal pagination envelope, returns `items: []`, and sets `total` to the number of matching rows. Use `/count` when you only need `{ "count": N }`.
